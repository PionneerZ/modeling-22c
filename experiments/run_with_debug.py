...
def _write_notes(path: Path, config: dict, run_id: str, run_path: Path, data_info: dict | None) -> None:
...
def main() -> None:
    ...
    parser.add_argument("--run-id", default=None, help="override run_id for outputs/")
    ...
    results_df, trades_df, w_stats, debug_df, events_df = run_backtest(
    ...
    debug_df.to_csv(debug_dir / "strategy_debug.csv", index=False)
    events_df.to_csv(debug_dir / "state_events.csv", index=False)
    debug_df[["t", "date"]].drop_duplicates().sort_values("t").to_csv(
    ...
    _write_notes(out_dir / "notes.md", config, run_id, out_dir, data_info)
